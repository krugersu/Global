...
qrSelect_last_workshift_date = 'SELECT MAX(workshiftid) AS "MaxDate" FROM workshift'
...
qrSelect_workshift_open = 'SELECT shiftnum ,cashcode,CAST(time_beg AS char),shopcode  FROM workshift WHERE time_end IS NULL AND workshiftid >%s  '
...
qrSelect_workshift = '''SELECT shiftnum , shopcode, CAST(time_end AS char) , cashcode, CAST(time_beg AS char), workshiftid, storeId,cashId, scode,
                        ...
                        FROM workshift WHERE  workshiftid >%s '''
